...
def run_full_optimization_flow(platform: SimulatePlatform, initial_parameters: dict, circuit_graph: CircuitGraph, max_sim_count: int, args):
    ...
    try:
        ...
        # ======================== 中场检查与决策 ========================
        final_tracking_info = pass1_tracking_info # 默认最终结果是 Pass 1 的
        
        print("\n--- INTERMISSION CHECK ---")

        # [MODIFIED LOGIC] 优先检查是否存在一个满足3个约束的“候选解”
        if candidate_info['best_params'] is not None and simulation_count < max_sim_count:
            print(f"A special candidate meeting 3 constraints was found (Score: {candidate_info['best_score']:.2f}).")
            print("This candidate will be used to start Pass 2, overriding the global best from Pass 1 if necessary.")
            
            starting_params_for_pass2 = candidate_info['best_params']
            gain_db_best_val = candidate_info['best_metrics'].get('Gain_db', 0)
            
            new_gain_target = round(gain_db_best_val / 10.0) * 10.0
            print(f"Original Gain_db from candidate was {gain_db_best_val:.2f}. New target for Pass 2: {new_gain_target:.2f} dB")

            # Proceed to Pass 2 using this candidate
            ...
            # [ORIGINAL LOGIC AS FALLBACK] 如果没有找到特殊候选，再按原逻辑检查全局最优解
            ...
            else: # Covers constraints_met < 4 (including 3, but we know it's not the special candidate case)
                print("No special candidate found and global best did not meet 3 constraints. Optimization halted.")
