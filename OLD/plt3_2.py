...
def extract_performance_data(filename="tmp6_2.txt"):
    # ... (no changes needed)
...
def plot_performance_data(results, output_dir="performance_plots", start_iter=1, end_iter=None):
    ...
    # 3. 准备绘图
    performance_data = dict(zip(["UGB", "Phase_Margin", "Gain_db", "Gain_Margin", "I_OPA", "Total_Area"], data_sliced))
    performance_data["Objective_Score"] = objective_score_list
    ...
    for name, data_list in performance_data.items():
        if name == "Objective_Score":
            ...
            fig.suptitle(f"Performance Trend: {name} (Iter {start_iter} to {end_iter_safe})")
            ...
            if np.any(feasible_mask):
                ...
                ax1.set_ylim(min_val - margin, max_val + margin)
                 ...
                 ax1.text(0.5, 0.5, 'No Feasible Solutions Found in this Range', 
            ...
            if np.any(infeasible_mask):
                ax2.plot(iterations[infeasible_mask], data_array[infeasible_mask], 'o-', color='r', label='Infeasible (Penalty)', markersize=4)
            ...
            ax.set_title(f"Performance Trend: {name} (Iter {start_iter} to {end_iter_safe})")
            data_array = np.array(data_list, dtype=np.float64)
            valid_mask = ~np.isinf(data_array) & ~np.isnan(data_array)
            # ... (此处省略了标准绘图代码，保持不变即可)
            # 为了简洁，您可以从我们上一版的代码中复制这部分标准绘图逻辑
            plot_configs = {"UGB": {"log": True}, "I_OPA": {"log": True}, "Total_Area": {"log": True}} # 简化版
            if name in plot_configs and plot_configs[name].get("log"):
                valid_mask &= (data_array > 0)
            if np.count_nonzero(valid_mask) == 0: plt.close(); continue
...
if __name__ == "__main__":
    results = extract_performance_data("tmp6_2.txt")
    if results:
        plot_performance_data(results, output_dir="performance_plots_v5_2")