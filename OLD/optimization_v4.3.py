...
def run_dynamic_optimization_v4_3(platform: SimulatePlatform, initial_parameters: dict,
                                  circuit_graph: CircuitGraph,
                                  perturb_ratio: float = 0.05,
                                  greedy_threshold_pct: float = 5.0,
                                  max_sim_count: int = 500): 
    """
    执行一个两阶段的动态收敛优化算法 (V4.3)。
    ...
    """
    print("\n===================================================================")
    print("===   DYNAMIC TWO-STAGE OPTIMIZATION (V4.3)               ===")
    print("===================================================================")
    ...
    def check_constraints(scores: dict) -> bool:
        """检查是否满足所有硬性约束"""
        ...
        iopa_ma = scores.get('I_OPA', 1.0) * 1000.0
        return (pm >= 50.0 and gain >= 80.0 and gm <= -10.0 and iopa_ma <= 3.0)
    ...
    frozen_params_stage2 = set()  # 阶段二参数冻结集合
    constraints_ever_met = check_constraints(eval_results)  # 是否曾经满足过所有约束
    ...
    while simulation_count < max_sim_count:
        ...
        eval_results = platform.evaluate(); simulation_count += 1
        score_current = get_score(eval_results)
        constraints_met = check_constraints(eval_results)
        constraints_ever_met = constraints_ever_met or constraints_met
        
        print(f"  - Current score: {score_current:.4f} (Sim count: {simulation_count})")
        print(f"  - Constraints {'satisfied' if constraints_met else 'violated'}")
        
        # 如果所有参数都被冻结，退出阶段二
        active_params = list(other_param_names - frozen_params_stage2)
        if not active_params:
            print(f"\n--- Stage 2 CONVERGENCE: All parameters have been frozen. ---")
            break
        ...
        for name in shuffled_param_names:
            ...
            for sign in directions:
                ...
                probe_eval_results = platform.evaluate(); simulation_count += 1
                score_probe = get_score(probe_eval_results)
                probe_constraints_met = check_constraints(probe_eval_results)
                
                # 更新约束满足状态
                constraints_ever_met = constraints_ever_met or probe_constraints_met
                ...
                # 根据不同阶段使用不同的改进判断标准
                improvement_threshold = 0.0001  # 最小改进阈值
                if not constraints_ever_met:
                    # 在约束违例阶段，任何降低违例的变化都是改进
                    is_improvement = score_probe < score_current
                else:
                    # 在性能优化阶段，需要显著改进才接受
                    is_improvement = (score_probe < score_current and 
                                   (score_current - score_probe) > improvement_threshold)
                
                if is_improvement:
                    found_improvement = True
                    if score_probe < best_neighbor_so_far['score']:
                        best_neighbor_so_far = {
                            'params': copy.deepcopy(params_probe),
                            'score': score_probe
                        }
            ...
            if not found_improvement:
                print(f"  - Parameter '{name}' has been frozen (no improvement found).")
        ...
        if best_neighbor_so_far['params'] is not None:
            print(f"  - Moving to best neighbor with score {best_neighbor_so_far['score']:.4f}")
    ...
    final_result_file = f"{output_dir}/dynamic_v4.3_final_solution.txt"
    with open(final_result_file, 'w') as f:
        f.write(f"Optimization Status: Finished Dynamic Two-Stage Optimization (V4.3).\n")
        ...
        f.write(f"Constraints Ever Met: {constraints_ever_met}\n\n")
        f.write(f"Frozen 'm' parameters ({len(frozen_params)}): {sorted(list(frozen_params))}\n")
        f.write(f"Frozen 'fw/l/r/c' parameters ({len(frozen_params_stage2)}): {sorted(list(frozen_params_stage2))}\n\n")
        ...
        for name in sorted(all_optimizable_names):
            if name in final_best_params:
                ...
                status = ""
                if name in frozen_params: # 检查 Stage 1 冻结的参数
                    status = " (Frozen Stage 1)"
                elif name in frozen_params_stage2: # 检查 Stage 2 冻结的参数
...
if __name__ == "__main__":
    ...
    if args.set_params and not args.evaluate:
        ...
        run_dynamic_optimization_v4_3(platform, initial_parameters, circuit_graph,
                                    perturb_ratio=0.05,
