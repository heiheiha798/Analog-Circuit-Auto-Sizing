...
def run_managed_optimization_flow(platform: SimulatePlatform, initial_parameters: dict,
    ...
    # 为了最终输出，我们需要一个完整的参数名称列表
    m_param_names_final = {name for name in initial_parameters.keys() if name.endswith('_m')}
    other_param_names_final = {name for name in initial_parameters.keys() if not name.endswith('_m')}

    finalize_and_save_results(
        tracking_info=global_best_tracking_info,
        platform=platform,
        simulation_count=simulation_count,
        frozen_params=set(), # 最终报告中不特别区分冻结参数
        m_param_names=m_param_names_final,
        other_param_names=other_param_names_final,
